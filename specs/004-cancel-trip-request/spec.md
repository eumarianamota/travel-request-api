...
**Independent Test**: Persist a travel request with status `requested`,
...
1. **Given** a travel request exists and is currently `requested`, **When** the
...
- **FR-006**: The cancellation operation MUST preserve observable statuses and
  limit them to `requested` and `canceled`.
...
- The current observable statuses remain limited to `requested` and `canceled`.
