...
class InMemoryTripRequestRepository implements TripRequestRepository {
  ...
  public async create(_input: TripRequestDraft): Promise<TripRequest> {
    throw new Error('not implemented in cancel conflict test')
  }
...
describe('PATCH /trip-requests/:id/cancel conflict flow', () => {
  it('returns 409 when the trip request is already canceled and preserves stored state', async () => {
    ...
    await withTestServer(app, async (baseUrl: string) => {
      ...
      await expect(response.json()).resolves.toStrictEqual({
        ...
        error: {
          ...
          message: 'Travel request is already canceled.',
        },
      })
